                                    ...
                                    <table class="table table-bordered data-table table table-hover" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                            ...
                                            $get= mysqli_query($c, "SELECT pb.id_peminjaman, u.nama AS nama_user, b.kode_barang, b.nama_barang, pb.jumlah, DATE_FORMAT(pb.tanggal_peminjaman, '%d-%m-%Y') AS tanggal_peminjaman, DATE_FORMAT(pb.tanggal_pengembalian, '%d-%m-%Y') AS tanggal_pengembalian, pb.jaminan, pb.status
                                                        ...
                                                        where u.nama = '$nama_user' and pb.status != 'Dikembalikan'");
                                            }
                                            else{
                                                $get= mysqli_query($c, "SELECT pb.id_peminjaman, u.nama AS nama_user, b.kode_barang, b.nama_barang, pb.jumlah, DATE_FORMAT(pb.tanggal_peminjaman, '%d-%m-%Y') AS tanggal_peminjaman, DATE_FORMAT(pb.tanggal_pengembalian, '%d-%m-%Y') AS tanggal_pengembalian, pb.jaminan, pb.status
                                                FROM peminjaman_barang pb
                                                INNER JOIN user u ON pb.id_user = u.id_user
                                                INNER JOIN barang b ON pb.id_barang = b.id_barang where pb.status != 'Dikembalikan'"); 
                                            }
                                            $i = 1;
            ...
            <form method="post">
                <div class="modal-body">
                    ...
                    <div class="jumlah mt-3">
                        ...
                        <input type="number" name="jumlah" class="form-control mt-0" placeholder="0" min="1" required>
                    </div>
